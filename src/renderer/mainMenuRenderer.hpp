        ...
        void render(
            ...
            case opt::TITLE:
                if (_state.selected == opt::TITLE)
                {
					...
					atlas.load(renderer.get(), "../resources/assets/Atlas/atlas.bmp", "../resources/assets/Atlas/atlas.xml");
					atlas.draw(renderer.get(), "player/bow1", 0.0, 0.0, 10.0f);
                }
                else
                {
            ...
            case opt::OPTIONS:
                fWriteLine(_state.inputDevicePolls.empty() ? 
                    "Plug in a gamepad, please." : "Connected gamepads:");
